...
function Card({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-2xl shadow-md border border-gray-200 p-5 bg-white">
      {children}
    </div>
  );
}
...
function Button({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`rounded-xl px-4 py-2 text-sm transition ${base} disabled:opacity-50 disabled:cursor-not-allowed`}
    >
      {children}
    </button>
  );
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold">
            AI Wallet Assistant — Risk Check
          </h3>
          <p className="text-sm text-gray-600">
            Paste a contract address. We’ll run client-side heuristics and show
            a risk hint. (Demo build)
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
            <div className="rounded-xl border p-3">
              ...
              <ul className="list-disc list-inside text-gray-700 space-y-1">
                ...
                <li>
                  Has transferFrom(): {String(result.facts.hasTransferFrom)}
                </li>
                <li>
                  Recent deploy: {String(result.facts.recentDeploy)} (demo)
                </li>
              </ul>
            </div>
            <div className="rounded-xl border p-3">
              ...
              {result.score >= 60 ? (
                <p className="text-red-700">
                  High risk. Avoid granting allowances or signing until you
                  verify the code and team.
                </p>
              ) : result.score >= 25 ? (
                <p className="text-amber-700">
                  Medium risk. Start with tiny amounts; revoke allowances after
                  use.
                </p>
              ) : (
                <p className="text-green-700">
                  Low risk (heuristic). Still verify on explorers before large
                  funds.
                </p>
              )}
            </div>
    ...
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">
          Rugsense — On-Chain AI Wallet Assistant
        </h1>
                ...
                <div className="space-y-4">
                  <Card>
                    <div className="text-sm text-gray-700 space-y-2">
                      ...
                      <p>
                        We connect your wallet (RainbowKit + wagmi), run a few
                        on-chain heuristics (via viem), and produce a risk hint.
                        In a full build, we add Tenderly simulations and LLM
                        reasoning.
                      </p>
